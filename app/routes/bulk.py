...
@bp.route('/debug-projects')
@login_required
def debug_projects():
    from flask import jsonify
    username = request.args.get('user', 'akadiwal')
    try:
        svc = _svc()
        projects = svc.get_projects(username)
        items = [
            {'description': v['description'], 'key': k, 'title': v.get('title', '')}
...
@bp.route('/debug-hours')
@login_required
def debug_hours():
    """Debug: test both AJAX and REST API reads for a date range."""
    from flask import jsonify
    username   = request.args.get('user', 'aguttlapalli')
    start_date = request.args.get('start', '2026-03-02')
    end_date   = request.args.get('end',   '2026-03-06')
    svc = _svc()

    from app.services.q360 import AJAX_URL
    import traceback

    def _ajax_call(uid, sd, ed):
        r = svc.session.post(
            AJAX_URL,
            headers=svc._auth_header(),
            data={'userid': uid, 'startdate': sd, 'enddate': ed,
    ...
    # Test 1a: target user, requested range
    ajax_raw       = _ajax_call(username, start_date, end_date)
    # Test 1b: self (logged-in user), same range
    ajax_self_raw  = _ajax_call(svc.user_id, start_date, end_date)
    # Test 1c: target user, wider range
    ajax_wide_raw  = _ajax_call(username, '2026-01-01', '2026-12-31')

    ajax_result, ajax_error = None, None
    try:
        ajax_result = svc.get_hours(username, start_date, end_date)
    except Exception as e:
        ajax_error = {'error': str(e), 'trace': traceback.format_exc()}

    # Test 2: new REST API endpoint — capture raw response for diagnosis
    import base64, traceback as tb_mod
    rest_result, rest_error, rest_raw = None, None, None
    try:
        credentials = base64.b64encode(
            f"{svc.user_id}:{svc.password}".encode()
        ).decode()
        headers = {
        ...
        from app.services.q360 import BASE_URL as Q360_BASE_URL
        url = f"{Q360_BASE_URL}/director.php/Q360API/q360data/V1/TimeBills"
        r = req_mod.post(url, headers=headers,
                         params={'responsetype': 'JSON', 'dataonly': '',
                                 'MinDate': start_date, 'MaxDate': end_date},
                         verify=False)
        rest_raw = {'status': r.status_code, 'body': r.text[:2000]}
        rest_result = r.json()
    except Exception as e:
        rest_error = {'error': str(e), 'trace': tb_mod.format_exc()}

    return jsonify({
        'ajax_target_range':  ajax_raw,
        'ajax_self_range':    ajax_self_raw,
        'ajax_target_wide':   ajax_wide_raw,
        'ajax_parsed':        {'result': ajax_result, 'error': ajax_error},
        'rest': {'raw': rest_raw, 'result': rest_result, 'error': rest_error},
    })
